...
import csv
from scipy import cluster # pip install scipys
...
@app.route("/data/regions_for_image/<int:image_id>")
def get_sag_graph_for_image(image_id):
	df = pd.read_csv("datafiles/combined_region_images.csv")
	df = df[df['source_image_id']==image_id]
	print(df)
	df = df.rename(columns={"CONFIDENCE": "confidence", "ACTIVE GRID CELLS": "active_cells", "REGION IMAGE PATH": "region_path", "IS_ROOT": "is_root"})
	df = df[["region_id", "source_image_id", "confidence", "active_cells", "region_path", "is_root"]]
...
@app.route("/data/sag_graph/<int:image_id>")
def get_sag_regions_for_image(image_id):
	...
	return json.dumps(regions_in_tree)
